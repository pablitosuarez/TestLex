...
@Name("cargadorBolilleroEx")
public class CargadorBolilleroEx extends Controller{
...
//	private TipoOficina tipoOficinaJuzgadoNacional;
	...
	public static CargadorBolilleroEx instance() {
		return (CargadorBolilleroEx) Component.getInstance(CargadorBolilleroEx.class,true);
	}
...
//	}
	...
	public void cargaBolillero(Sorteo sorteo, TipoBolillero tipoBolillero, List<Oficina> oficinas, List<String> rubros, int bolasPorOficina, Materia materia, boolean doBlanqueo){		
		...
		for(Oficina oficina: oficinas){
			List<String> rubrosDeLaOficina = getRubrosOficina(oficina, rubros);
			for(String rubro: rubrosDeLaOficina){
	...
	private List<String> getRubrosOficina(Oficina oficina, List<String> rubrosSorteo) {
		List<String> ret = SorteoHome.getRubrosAsList(oficina.getRubros());
		if(ret == null || ret.size() == 0){
			ret = rubrosSorteo;
		}else{
			ret = intersection(rubrosSorteo, ret);
		}
		return ret;
	}

	private boolean oficinaContaisRubro(Oficina oficina, String rubro) {
		boolean ret = true;
		if(!Strings.isEmpty(oficina.getRubros())){
			List<String> list = SorteoHome.getRubrosAsList(oficina.getRubros());
			ret = list.contains(rubro);
		}
		return ret;
	}
	...
	public List<StatusBolillero> verificaBolillero(Sorteo sorteo, TipoBolillero tipoBolillero, List<Oficina> oficinas, List<String> rubros, int bolasPorOficina, Materia materia, boolean returnOnlyErrors){
		...
		for(String rubro: rubros){
			...
			for(Oficina oficina: oficinas){
				int num = bolasPorOficina < 1 || bolasPorOficina > MAX_BOLAS_POR_OFICINA ? 1 : bolasPorOficina;
				if(!oficinaContaisRubro(oficina, rubro)){
					num = 0;
				}
				List<OficinaCargaExp> items = findOficinaCargaExp(sorteo, tipoBolillero, oficina, rubro);
				if(items.size() == 0){
	...
	private void borrarRubro(Sorteo sorteo, String rubro) {
		List<OficinaCargaExp> items = findOficinaCargaExp(sorteo, rubro);
		for(OficinaCargaExp item: items){
			item.setLogicalDeleted(true);
	...
	private List<OficinaCargaExp> findOficinaCargaExp(Sorteo sorteo, String rubro) {
		String stmt = "from OficinaCargaExp jce where sorteo = :sorteo and rubro = :rubro and status <> -1 order by cargaTrabajo";
		Query query = entityManager.createQuery(stmt);
